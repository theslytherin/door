...
class Fullscreen_Window:
        ...
        def res(self):
            self.label = Label(self.tk, text=" Dear Resident, Please enter your token:", font=('Helvetica 15'))
            self.label.grid(row=1, column=0)
            self.label.config( justify='center', anchor='center')
        ...
        def guest(self):
            self.label = Label(self.tk, text=" Guest", font=('Helvetica 15'))
            self.label.config( justify='center', anchor='center')
        ...
        def show_idle1(self):
            ...
            self.welcomeLabel.config(font='size, 15', justify='center', anchor='center')
            self.welcomeLabel.grid(sticky=tk.W+tk.E, pady=210)
            
            self.button = tk.Button(self.tk, 
                   text="Resident")
            self.button['command']=self.res
                   
            self.button.grid(row = 0, column = 1, padx = 30, pady=100)
            self.slogan = tk.Button(self.tk,
                   text="Guest",
                   command=self.guest)
        ...
        def guestlogin(self):
                self.label1 = Label(self.tk, text=" Dear Guest, your request has been sent!", font=('Helvetica 15'))
                self.label1.grid(row=1, column=0)
                self.label1.config( justify='center', anchor='center')
            
                self.label.grid_forget()
                url = 'https://www.pushsafer.com/api'
                post_fields = {
                    "t" : 'Alert',
	            "m" : 'Guest is trying to enter',
	            "s" : 11,
	            "v" : 3,
	            "i" : 33,
                    "a" : 1,
	            ...
	            "ut" : 'Open Pushsafer',
	            "k" : 'nFGJRKWlF5eOgXPIgw4J',
                    }

                request = Request(url, urlencode(post_fields).encode())
                json = urlopen(request).read().decode()
                print(json)
                self.image = tk.PhotoImage(file="/home/root1/Downloads/pic2.png")
                self.photoLabel = ttk.Label(self.tk, image=self.image)
                self.photoLabel.grid(columnspan=3)
